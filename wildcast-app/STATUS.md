...
- Not yet manually verified against a live deployment (no working local dev server for the Blob-backed API layer) — logic verified by reading the full request/response wiring across `api/publish-template.js`, `src/lib/customTemplates.js`, `src/components/TemplatePicker.jsx`, and `src/components/TemplateImportPage.jsx`; a real click-through on the deployed app after push is the actual verification step here.
