...
### Briefing Form (added 2026-08-03) — new front page
The homepage no longer opens on the 3-step template picker — it opens on a full briefing form (two-column layout: hero copy left, form right), matching how a partner would brief a human agency.
- 11 fields: Partner name (existing/new), Business type, "what's this about", Objective (with per-choice follow-up), Formats needed, Restaurant name (shown once Flyer is picked, defaults to Partner name), Headline + Subline (each with a "✦ AI Suggest" demo — canned DE/EN suggestions, no real API call), Call to action (Option B's second line only — optional, falls back to Subline then the objective text if left blank), Sticker/QR/Food photo (all pull from the real shared Library via a new reusable picker, "request a new sticker" fallback), T&Cs.
