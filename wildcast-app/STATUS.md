...
### Briefing Form (added 2026-08-03, restructured 2026-08-06) — new front page
The homepage no longer opens on the 3-step template picker — it opens on a full briefing form (two-column layout: hero copy left, form right), matching how a partner would brief a human agency. As of 2026-08-06 the form itself is paginated into 3 steps (basics → copy → assets/submit) with Next/Back, instead of one long scroll — see the 2026-08-06 update entries below for the full build log.
- 11 fields across the 3 steps: Partner name (existing/new), Business type, "what's this about", Objective (with per-choice follow-up), Formats needed, Restaurant name (shown once Flyer is picked, defaults to Partner name) — **Step 1**; Headline + Subline (each with "Write my own" / "Choose preset" / "AI Suggest" as one button row — see below), Call to action (Option B's second line only — optional, falls back to Subline then the objective text if left blank) — **Step 2**; Sticker/QR/Food photo (pull from the real shared Library via a reusable picker, now with a direct "Upload from computer" option too — same transparency rules as the rest of the app, see 2026-08-06 entry), T&Cs, Submit — **Step 3**.
- **Headline/Subline copy tools (real as of 2026-08-06, was a demo before):** "Choose preset" pulls real historical Wolt campaign copy straight from a published Google Sheet (`api/presets.js`), scoped to the selected partner's own past campaigns where the library has any (falls back to the full list otherwise). "AI Suggest" (`api/ai-suggest.js`) generates new tailored copy via the real Claude API, grounded in that same sheet as style reference, respecting the same character limits FieldEditor.jsx already enforces (headline 30 / sub_headline 60 / etc.) so suggestions actually fit the template zones instead of reading as a combined headline+tagline.
- Submitting maps the answers onto the two live templates (Restaurant Flyer Option A & B — the only live inventory today) and renders both as real filled preview cards on a "Pick a design" screen. If Business type/Formats don't match anything live yet (e.g. Retail), shows a plain read-back instead of pretending candidates exist.
- Tick one or both candidates, then either **Send for Review** (skips the editor entirely, saves straight from the already-rendered preview — ticking both sends two independent designs/review links) or **Edit** (only enabled with exactly one ticked).
- Edit opens a new **restricted review mode**: Headline/Sub-headline/Offer get nudge (position) + scale (font size, capped to ±20% of the auto-generated size) but the text itself is read-only; every other field is fully locked; images can't be replaced (upload/library-pick hidden), only repositioned/rescaled. Footer has **Save & pick another design** (persists via the normal save-project/Designs mechanism, then returns to "Pick a design" — so an interrupted session isn't lost and a merchant wanting both A and B isn't stuck on one) and **Send for Review**. Export PDF is hidden here.
- Known gaps, not bugs: Option B has no `restaurant_name`/`tc`/`qr` zone (never shows the partner's name); no live template has a `qr` zone at all yet; Sticker/QR/Food-photo pickers only show real assets once something's been uploaded to that folder via the Library page — same real Library, just often empty right now; Sticker specifically has nowhere to render at all yet, since neither Option A nor B has a sticker zone defined.
- A "+ New Brief" nav item (top header) starts a genuinely fresh brief from anywhere.
- Re-clicking Edit on a candidate already saved once (via Save & pick another design) now reopens that exact saved state — same font sizes/positions/project id — instead of a fresh, un-edited regeneration.
- See the 2026-08-03 update entries below for the fuller build log (5 rounds of live-tested fixes same day, including a real root-cause font-size bug — auto-shrink was never synced back into React state, causing wild size jumps that looked like cross-field coupling — and a real root-cause "form jumps to templates" bug, see Round 5).
