...
- 10 fields: Partner name (existing/new), Business type, "what's this about", Objective (with per-choice follow-up), Formats needed, Restaurant name (shown once Flyer is picked, defaults to Partner name), Headline + Subline (each with a "✦ AI Suggest" demo — canned DE/EN suggestions, no real API call), Sticker/QR/Food photo (all pull from the real shared Library via a new reusable picker, "request a new sticker" fallback), T&Cs.
