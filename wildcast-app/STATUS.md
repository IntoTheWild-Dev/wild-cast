...
| **Export frame looked bigger/different than the Figma master (2026-07-24)** | Investigated with real pixel measurements, not assumption: the export is correctly sized (1311×1819px @300dpi = exactly 111×154mm, confirmed via `pdfinfo`) — what Julia was seeing is the real 3mm bleed margin (required for print, gets trimmed off) plus the ~6-7mm safe margin already baked into the source art, both invisible as "margin" in a flat exported PDF with no guide overlay. The new trim-line guide above should make this obvious going forward. Separately, DID fix a real (if minor, ~1%) issue found during this investigation: the editor canvas (316×441px) is ~1% off true A6's 105:148 ratio (a historical rounding artifact — canvasH was set to 441 rather than the more precise 445), so export's `fit:'fill'` resize was stretching every element slightly non-uniformly. Switched to `fit:'cover'`, which crops the negligible overflow instead of distorting proportions. |
