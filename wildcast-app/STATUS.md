...
- Known gaps, not bugs: Option B has no `restaurant_name`/`tc`/`qr` zone (never shows the partner's name); no live template has a `qr` zone at all yet; Sticker/QR/Food-photo pickers only show real assets once something's been uploaded to that folder via the Library page — same real Library, just often empty right now.
- See the 2026-08-03 update entries below for the fuller build log (4 rounds of live-tested fixes same day, including a real root-cause font-size bug — auto-shrink was never synced back into React state, causing wild size jumps that looked like cross-field coupling).
